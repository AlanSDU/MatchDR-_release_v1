...
%     for k = 0:KMax
        ...
        [X2] = Neighbour(D1,D2,X1,C,alpha,beta);
        Delta_J = J(X2) - J(X1);
...
%     end
    ...
    temperature = temperature-T/10000;
...
function [X] = Neighbour(D1, D2, X0, C, alpha, beta)
...
%     J = @(x) trace((Q(x) - sum(sum(Q(x)))/(sum(x(:))^2).*(Q(x)>0))' * (Q(x) - sum(sum(Q(x)))/(sum(x(:))^2).*(Q(x)>0)));
    ...
    dX = 4/SumX^2 * t1 * t2 - 2*sum(sum(Q.^2))/SumX^3 * X0 - 2*(size(D1,1)*size(D2,1))*SumQ^2/SumX^7 * X0 ...
        ...
        +beta * repmat((X0*C{1}-C{2}), 1, size(C{1},1)) .* t4;
